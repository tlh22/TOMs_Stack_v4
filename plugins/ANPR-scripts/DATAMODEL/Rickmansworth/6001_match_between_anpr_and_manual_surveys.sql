...
BEGIN
    ...
    LOOP
		...
		-- Assume each vehicle is parked in only one permit type
		
		SELECT DISTINCT ON ("VRM") "VRM", "VehicleTypeID", "PermitTypeID"
		FROM demand."VRMs" WHERE "RoadName" = car_park."Description"
		...
		LOOP
			...
			IF vehicle_type_id IS NOT NULL OR permit_type_Id IS NOT NULL THEN
				...
				WHERE "ID" IN (obs."ID", obs."MatchedTo");
...
-- Reset details
UPDATE anpr."VRMs" v
SET "VehicleTypeID" = NULL,  "PermitTypeID" = NULL
FROM (
...
UPDATE anpr."VRMs" v
SET "VehicleTypeID" = y."VehicleTypeID",  "PermitTypeID" = y."PermitTypeID"

--SELECT v."VRM", y."VehicleTypeID",  y."PermitTypeID"
--FROM anpr."VRMs" v,
FROM 
	(
	SELECT "SiteID", cp."CarParkID", cp."Description"
	...
	(
	SELECT DISTINCT ON ("VRM") "VRM", "RoadName", "VehicleTypeID", "PermitTypeID"
	FROM demand."VRMs" dv, mhtc_operations."Supply" s 
	WHERE dv."GeometryID" = s."GeometryID") y
	
WHERE v."SiteID" = x."SiteID"
AND x."Description" = y."RoadName"
AND y."VRM" = v."VRM"
AND x."CarParkID" = x
