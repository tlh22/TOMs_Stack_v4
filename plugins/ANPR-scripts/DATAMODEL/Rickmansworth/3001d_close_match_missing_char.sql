...
BEGIN
    ...
    LOOP
		...
		FOR obs IN SELECT "FromID", "FromVRM", "FromCaptureTime", "FromDirection", "ToSiteID", "ToID", "ToVRM", "ToCaptureTime", "FromDirection", "Duration", "Difference"
				   FROM					  
				  (SELECT v1."ID" AS "FromID", v1."VRM" AS "FromVRM", v1."CaptureTime" AS "FromCaptureTime", v1."Direction" AS "FromDirection",
						  ...
						  levenshtein(REPLACE(v1."VRM", '-', ''), REPLACE(v2."VRM", '-', ''), 1, 1, 1) AS "Difference",
				   ...
				   WHERE v1."ID" != v2."ID"
				   AND levenshtein(REPLACE(v1."VRM", '-', ''), REPLACE(v2."VRM", '-', ''), 1, 1, 1) <= 1  
		...
		LOOP
			...
			IF not_updated_to = true AND not_updated_from = true THEN
