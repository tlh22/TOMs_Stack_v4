-- Output of duration for ALL vehicles with vehicle type and permit type

--DEALLOCATE usrrptplan;

PREPARE usrrptplan (int, int) AS

-- $1 = Default permit type
-- $2 = car park nr
...
FROM (
		...
		(SELECT v."ID", s."CarParkID", v."SiteID", v."VRM"
		 ...
		 FROM (SELECT "ID", "SiteID", "VRM", "CaptureTime", "VehicleTypeID", "MatchedTo"
					  , CASE WHEN "PermitTypeID" = 0 OR "PermitTypeID" IS NULL THEN $1  -- default permit type *** need to change !!!
							 ELSE "PermitTypeID"
						END AS "PermitTypeID"	
			   FROM anpr."VRMs" 
			   ) v
		 ...
		 WHERE v."SiteID" = s."SiteID"
		  ...
		  , anpr."CarParks" c
			WHERE v1."CarParkID" = $2  -- car parking being considered *** need to change !!!
