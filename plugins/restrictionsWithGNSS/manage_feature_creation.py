...
class captureGPSFeatures(FieldRestrictionTypeUtilsMixin):
    ...
    def __init__(self, iface, featuresWithGPSToolbar):
        ...
        TOMsMessageLog.logMessage("In captureGPSFeatures::init", level=Qgis.MessageLevel.Info)
    ...
    def enableFeaturesWithGPSToolbarItems(self):
        ...
        TOMsMessageLog.logMessage("In enablefeaturesWithGPSToolbarItems", level=Qgis.MessageLevel.Warning)
        ...
        TOMsMessageLog.logMessage("In enableFeaturesWithGPSToolbarItems: GPS port is: {}".format(gpsPort), level=Qgis.MessageLevel.Warning)
        self.gpsConnection = None
        ...
        if self.gpsAvailable == True:
            ...
            try:
                ...
                TOMsMessageLog.logMessage("In enableFeaturesWithGPSToolbarItems:init: roamDistance issue: {}".format(e), level=Qgis.MessageLevel.Warning)
    ...
    def enableToolbarItems(self):
        TOMsMessageLog.logMessage("In enableToolbarItems", level=Qgis.MessageLevel.Warning)
    ...
    def disableFeaturesWithGPSToolbarItems(self):
        ...
        TOMsMessageLog.logMessage("In disablefeaturesWithGPSToolbarItems", level=Qgis.MessageLevel.Warning)
    ...
    def onGroupTriggered(self, action):
        ...
        TOMsMessageLog.logMessage("In onGroupTriggered: curr action is {}".format(action.text()), level=Qgis.MessageLevel.Info)
    ...
    def changeMapTool2(self):
        TOMsMessageLog.logMessage(
            "In changeMapTool2 ...", level=Qgis.MessageLevel.Info)

        currMapTool = self.iface.mapCanvas().mapTool()

        if not self.isGnssTool(currMapTool):
            TOMsMessageLog.logMessage(
                "In changeMapTool2. Unchecking action ...", level=Qgis.MessageLevel.Info)
            if self.currGnssAction:
                self.currGnssAction.setChecked(False)
        else:
            TOMsMessageLog.logMessage(
            "In changeMapTool2. No action for gnssTools.", level=Qgis.MessageLevel.Info)

        TOMsMessageLog.logMessage(
            "In changeMapTool2. finished.", level=Qgis.MessageLevel.Info)
        #print('tool unset')

    def changeCurrLayer2(self):
        TOMsMessageLog.logMessage("In changeLayer2 ... ", level=Qgis.MessageLevel.Info)
        ...
        """if self.isGnssTool(currMapTool):
            TOMsMessageLog.logMessage("In changeLayer2. Action triggered ... ", level=Qgis.MessageLevel.Info)
            self.currGnssAction.trigger()  # assumption is that there is an action associated with the tool
        else:
            TOMsMessageLog.logMessage(
            "In changeLayer2. No action for currentMapTool.", level=Qgis.MessageLevel.Info)"""

        TOMsMessageLog.logMessage(
            "In changeLayer2. finished.", level=Qgis.MessageLevel.Info)
        print('layer changed')


    def doCreateRestriction(self):

        TOMsMessageLog.logMessage("In doCreateRestriction", level=Qgis.MessageLevel.Info)
        ...
        if self.actionCreateRestriction.isChecked():
            ...
            TOMsMessageLog.logMessage("In doCreateRestriction - tool activated", level=Qgis.MessageLevel.Info)
            TOMsMessageLog.logMessage(
            ...
            if not self.createRestrictionMapTool:
                TOMsMessageLog.logMessage("In doCreateRestriction. creating new map tool", level=Qgis.MessageLevel.Info)
            ...
            TOMsMessageLog.logMessage("In doCreateRestriction. Here 1", level=Qgis.MessageLevel.Info)

            self.iface.mapCanvas().setMapTool(self.createRestrictionMapTool)

            TOMsMessageLog.logMessage("In doCreateRestriction. Here 2", level=Qgis.MessageLevel.Info)
            ...
            if not self.createRestrictionMapTool.isCapturing():
                if self.currLayer.isEditable() == True:
                    if self.currLayer.commitChanges() == False:
                        ...
                        TOMsMessageLog.logMessage("In doCreateRestriction: changes committed", level=Qgis.MessageLevel.Info)
            ...
            TOMsMessageLog.logMessage("In doCreateRestriction. Here 3", level=Qgis.MessageLevel.Info)

        else:

            TOMsMessageLog.logMessage("In doCreateRestriction - tool deactivated", level=Qgis.MessageLevel.Info)
        ...
        TOMsMessageLog.logMessage("In doCreateRestriction. Here 4", level=Qgis.MessageLevel.Info)
    ...
    def changeExtents(self):
        TOMsMessageLog.logMessage("In changeExtents ... ", level=Qgis.MessageLevel.Info)

    def doAddGPSLocation(self):

        # need to have a addPointFromGPS function within each tool

        TOMsMessageLog.logMessage("In doAddGPSLocation", level=Qgis.MessageLevel.Info)
        ...
        if self.gpsConnection:
            ...
            if self.curr_gps_location:
                try:
                    ...
                    TOMsMessageLog.logMessage("In doAddGPSLocation: Problem adding gnss location: {}".format(e), level=Qgis.MessageLevel.Warning)
    ...
    def doRestrictionDetails(self):
        ...
        """
        TOMsMessageLog.logMessage("In doRestrictionDetails", level=Qgis.MessageLevel.Info)
        ...
        if self.actionRestrictionDetails.isChecked():
            ...
            TOMsMessageLog.logMessage("In doRestrictionDetails - tool activated", level=Qgis.MessageLevel.Warning)
            ...
            TOMsMessageLog.logMessage("In doRestrictionDetails - tool deactivated", level=Qgis.MessageLevel.Warning)
    ...
    def doRemoveRestriction(self):
        ...
        TOMsMessageLog.logMessage("In doRemoveRestriction", level=Qgis.MessageLevel.Info)
        ...
        if self.currLayer.readOnly() == True:
            """reply = QMessageBox.information(None, "Information",
                                            "Could not start transaction on " + self.currLayer.name(), QMessageBox.StandardButton.Ok)"""
            TOMsMessageLog.logMessage("In doRemoveRestriction - Not able to start transaction ...", level=Qgis.MessageLevel.Info)
            self.actionRemoveRestriction.setChecked(False)
            return

        if self.actionRemoveRestriction.isChecked():

            TOMsMessageLog.logMessage("In doRemoveRestriction - tool activated", level=Qgis.MessageLevel.Warning)
            ...
            TOMsMessageLog.logMessage("In doRemoveRestriction - tool deactivated", level=Qgis.MessageLevel.Warning)
    ...
    def removeRestriction(self, closestLayer, closestFeature):
        ...
        if closestLayer.isEditable() == True:
            if closestLayer.commitChanges() == False:
                ...
                TOMsMessageLog.logMessage("In removeRestriction: changes committed", level=Qgis.MessageLevel.Info)
    ...
    def doCreateSign(self):
        ...
        TOMsMessageLog.logMessage("In doCreateSign", level=Qgis.MessageLevel.Info)
        ...
        if self.actionCreateSign.isChecked():
            ...
            TOMsMessageLog.logMessage("In doCreateSign - tool activated", level=Qgis.MessageLevel.Info)
    ...
    def doCreateMTR(self):
        ...
        TOMsMessageLog.logMessage("In doCreateMTR", level=Qgis.MessageLevel.Info)

        if self.actionCreateMTR.isChecked():

            TOMsMessageLog.logMessage("In doCreateMTR - tool activated", level=Qgis.MessageLevel.Info)
            ...
            TOMsMessageLog.logMessage("In doCreateMTR - tool deactivated", level=Qgis.MessageLevel.Info)
    ...
    def onLocalChanged(self, text):
        TOMsMessageLog.logMessage(
            "In generateFirstStageForm::selectionchange.  " + text, level=Qgis.MessageLevel.Info)
    ...
    def reinstateMapTool(self):
        ...
        TOMsMessageLog.logMessage("In reinstateMapTool ... ", level=Qgis.MessageLevel.Info)
    ...
    def doMoveFeatureToDifferentLayer(self):
        """
            Select point and then display details. Assume that there is only one of these map tools in existence at any one time ??
        """
        TOMsMessageLog.logMessage("In doMoveFeatureToDifferentLayer", level=Qgis.MessageLevel.Info)
        ...
        if self.actionMoveFeatureToDifferentLayer.isChecked():
            ...
            TOMsMessageLog.logMessage("In doMoveFeatureToDifferentLayer - tool activated", level=Qgis.MessageLevel.Warning)
            ...
            TOMsMessageLog.logMessage("In doMoveFeatureToDifferentLayer - tool deactivated", level=Qgis.MessageLevel.Warning)
